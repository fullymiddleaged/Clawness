...
export function parseGetResult(stdout) {
    const text = (stdout ?? "").trim();
    if (!text || text === "null")
        return null;
    let obj;
    ...
    return {
        ...
        title: typeof r.title === "string" ? r.title : undefined,
        content: r.content,
        fromLine: Number.isFinite(r.fromLine) ? r.fromLine : 1,
        lineCount: Number.isFinite(r.lineCount) ? r.lineCount : 1,
        id: typeof r.id === "string" ? r.id : undefined,
    };
}
/**
 ...
 */
export function makeMemoryCorpusSupplement(getCwd) {
    return {
