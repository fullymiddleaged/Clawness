 ...
 */
import { runSessionNotes, REORIENTATION_NOTE_HOOKS } from "./notes.js";
/** The short notice that leads the re-orientation, so the model knows why. */
export const COMPACTION_NOTICE = "[Clawness] The conversation was just compacted, so earlier detail may be gone. " +
 ...
 */
export function assembleReorientation(notes, marker) {
    if (!notes.length)
        return [];
    const out = [{ text: COMPACTION_NOTICE, key: `notice:${marker}` }];
