...
the real OpenClaw SDK (PASS, 0 breakages). What remains needs a live host — the inspector's
one open proof-gap (`before_tool_call` block/approval semantics) and the runtime field names
below (the inspector is static, so it can't see what values the host actually passes):
